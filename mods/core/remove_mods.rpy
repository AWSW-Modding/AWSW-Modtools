    ...
    window id "modmenu_remove" at alpha_dissolve:
        ...
        python:
            ...
            for modname in get_mods():
                renpy.ui.textbutton(modname, (Show("modmenu_remove_confirm", modname=modname, filename=False), Play("audio", "se/sounds/open.ogg")), hovered=Play("audio", "se/sounds/select.ogg"), yalign=y_pos, style="menubutton2")
                y_pos += 0.12


screen modmenu_remove_confirm(modname, filename) tag smallscreen2:
    modal True
